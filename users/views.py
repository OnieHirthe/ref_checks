...
def profile(request):
    ...
    return render(request, "users/profile_main.html", { "own_sections" : own_sections, "sections" : sections, "sees_online" : sees_online })
...
def profile_endpoint(request):
    ...
    if request.method == 'POST':
        ...
        if "form" in request.path or not profile_instance.all_required_filled():
            return render(request, "users/profile_form.html", {
...
def participation_endpoint(request):
    ...
    print("pp", participation_instance)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print("invalid participation")
            return render(request, "users/participation_form.html", {
                    "participation_form" : form })
    else:
        if "form" in request.path or not request.user.profile.is_registered():
            return render(request, "users/participation_form.html", {
...
@login_required_redirect
def author_endpoint(request, author_id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print("valid author!")
            return render(request, "users/author_form.html", context={"author_instance" : author_instance})
        else:
            print("invalid author!")
            print(form.errors)
            return render(request, "users/author_form.html", context={"author_form" : form})

    else:
        if "form" in request.path:
            return render(request, "users/author_form.html", context={"author_form" : AuthorForm(instance=author_instance)})
        else:
...
@check_stage("REPORT_SUBMISSION")
@login_required_redirect
def advisor_endpoint(request, author_id=None):
    ...
    if request.method == "POST":
        form  = AuthorForm(request.POST, instance=author_instance)
        if form.is_valid():
            form.instance.advisor = True
            author_instance = form.save()
            print("valid author!")
            return render(request, "users/advisor_form.html", context={"author_instance" : author_instance})
        else:
            print("invalid author!")
            print(form.errors)
            return render(request, "users/advisor_form.html", context={"author_form" : form})

    else:
        if "form" in request.path:
            return render(request, "users/advisor_form.html", context={"author_form" : AuthorForm(instance=author_instance)})
        else:
            if author_instance is not None:
                return render(request, "users/advisor_form.html", context={"author_instance" : author_instance}) 
            else:
